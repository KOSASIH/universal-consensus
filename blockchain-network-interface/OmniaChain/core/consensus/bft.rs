...
// Define the Byzantine Fault Tolerance (BFT) consensus algorithm
pub struct BFT {
    // Nodes
    nodes: Vec<Node>,
    // Primary node
...
impl Consensus for BFT {
    fn new(nodes: Vec<Node>, primary: NodeId, block_time: u64, block_reward: u64, storage_network: StorageNetwork) -> Self {
        BFT {
            nodes,
            primary,
            block_time,
    ...
    fn propose_block(&mut self, node: &Node) -> ConsensusResult {
        ...
        if node.id!= self.primary {
            return Err("Node is not the primary".to_string());
        }
...
// Implement the BFT algorithm's logic
impl BFT {
    fn update_primary(&mut self, new_primary: NodeId) {
