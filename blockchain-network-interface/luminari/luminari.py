...
from luminari_node import LuminariNode
from luminari_protocol import LuminariProtocol

class Luminari(BlockchainNetwork):
    def __init__(self, config_file='luminari_config.json'):
        self.config = self.load_config(config_file)
        self.network = self.create_network()
        self.galactic_nexus = GalacticConsensusNexus(self.network)
        self.nodes = self.create_nodes()
        self.protocol = LuminariProtocol(self.network, self.nodes)
    ...
    def create_nodes(self):
        ...
        for node_config in self.config['nodes']:
            node = LuminariNode(node_config['host'], node_config['port'])
            nodes.append(node)
        return nodes
