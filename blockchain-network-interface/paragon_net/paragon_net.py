...
from paragon_node import ParagonNode
from paragon_protocol import ParagonProtocol

class ParagonNet(BlockchainNetwork):
    def __init__(self, config_file='paragon_net_config.json'):
        self.config = self.load_config(config_file)
        self.network = self.create_network()
        self.galactic_nexus = GalacticConsensusNexus(self.network)
        self.nodes = self.create_nodes()
        self.protocol = ParagonProtocol(self.network, self.nodes)
    ...
    def create_nodes(self):
        ...
        for node_config in self.config['nodes']:
            node = ParagonNode(node_config['host'], node_config['port'])
            nodes.append(node)
        return nodes
...
if __name__ == '__main__':
    paragon_net = ParagonNet()
    paragon_net.start_network()
